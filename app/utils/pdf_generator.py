...
def _header_footer(canvas: Canvas, doc, title: str):
    ...
    canvas.drawRightString(width - 20, height - 26, f"Export: {datetime.now().strftime('%Y-%m-%d %H:%M')}")

    # Footer
    canvas.setFillColor(colors.grey)
    canvas.setFont("Helvetica", 8)
    canvas.drawCentredString(width / 2, 15, "EDUMANAGE Center • Generated by Backend")
    canvas.restoreState()
...
def generate_group_timetable_pdf(group_name: str, rows: list[dict]) -> str:
    ...
    day_map = {0: 'Dimanche', 1: 'Lundi', 2: 'Mardi', 3: 'Mercredi', 4: 'Jeudi', 5: 'Vendredi', 6: 'Samedi'}
    data = [["Jour", "Début", "Fin", "Cours"]]
    for r in rows:
        day_label = day_map.get(int(r.get('day', 0)), str(r.get('day', '')))
        data.append([day_label, r.get('start', ''), r.get('end', ''), r.get('course', '')])

    t = Table(data, colWidths=[40*mm, 30*mm, 30*mm, 80*mm])
    t.setStyle(TableStyle([
        ('BACKGROUND', (0,0), (-1,0), colors.lightgrey),
        ('FONTNAME', (0,0), (-1,0), 'Helvetica-Bold'),
        ('GRID', (0,0), (-1,-1), 0.5, colors.grey),
